...
class ProfileService {
    ...
    async editUserProfile(uid, aboutMe, career, imageInfo) {
        try {
            ...
            let result = {};
            SQL = `UPDATE PROFILES SET ABOUT_ME=?, CAREER=?, IMAGE_PATH=? WHERE P_UID=?`;
            if(aboutMe === undefined) aboutMe = '';
            if(career === undefined) career = '';
            if(imageInfo === undefined) imageInfo = '';
            console.log('service: editUserProfile', aboutMe, career, imageInfo.path);
            await this.db.promise().execute(SQL, [aboutMe, career, imageInfo.path, uid]);
